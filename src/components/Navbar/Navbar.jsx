...
export function Navbar() {
  ...
  return <header className={`navbar ${open ? 'menu-open' : ''}`}>
    ...
    <nav ref={mobileNavRef} className="mobile-nav" aria-label="Mobile navigation">{navigation.map((item, index) => <a key={item.href} href={item.href} onClick={close}><span>0{index + 1}</span>{item.label}</a>)}</nav>
  </header>;
}
